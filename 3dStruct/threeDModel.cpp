...
{	
	...
	if (p.glBuffer != NULL)
	{
		glBuffer = new GLuint[p.m_sizeOfGLBuffer];
		m_sizeOfGLBuffer = p.m_sizeOfGLBuffer;
		memcpy(glBuffer, p.glBuffer, sizeof(GLuint)*p.m_sizeOfGLBuffer);
	}
...
void ThreeDModel::initVBO(Shader* myShader)
{
	...
	m_sizeOfGLBuffer = 4+numOfMaterials;
	glBuffer = new GLuint[m_sizeOfGLBuffer];
	glGenBuffers(m_sizeOfGLBuffer, glBuffer);
